...
def test_handle_timew_errors_catches_timewarrior_error():
    ...
    with patch("time_helper.cli.utils.rprint") as mock_rprint:
        ...
        import time_helper.colors as colors

        expected_msg = f"[{colors.WARNING}]No data found for the specified timespan[/{colors.WARNING}]"
        mock_rprint.assert_called_with(expected_msg)
