...
def test_run_timew_command_raises_timewarrior_error():
    ...
    with patch("subprocess.run") as mock_run:
        ...
        cmd = ["timew", "args"]
        error = subprocess.CalledProcessError(1, cmd, output="output", stderr="Error message")
...
def test_run_timew_command_extracts_stdout_if_stderr_empty():
    ...
    with patch("subprocess.run") as mock_run:
        cmd = ["timew", "args"]
        error = subprocess.CalledProcessError(1, cmd, output="Error in stdout", stderr="")
        mock_run.side_effect = error
...
def test_handle_timew_errors_catches_timewarrior_error():
    """Test that handle_timew_errors catches TimewarriorError and prints message, then re-raises."""
    ...
    with patch("time_helper.cli.utils.rprint") as mock_rprint:
        ...
        mock_rprint.assert_called_with("[yellow]No data found for the specified timespan[/yellow]")

def test_handle_timew_errors_catches_timewarrior_error_unknown():
    """Test that handle_timew_errors catches unknown TimewarriorError and re-raises."""
    @handle_timew_errors
    def failing_func():
    ...
    with patch("time_helper.cli.utils.rprint") as mock_rprint:
        ...
        # Should NOT print unknown errors anymore, as the global handler does it
