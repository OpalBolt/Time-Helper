...
def test_format_as_markdown_exists():
    ...
    assert hasattr(
        generator, "format_as_markdown"
    ), "ReportGenerator should have 'format_as_markdown' method"
...
def test_format_as_markdown_content():
    ...
    tag_summary = TagSummary(
        tag="work", total_hours=1.0, entries=[entry], annotations=["Test task"]
    )

    daily_report = DailyReport(
        date=start_date, tag_summaries={"work": tag_summary}, total_hours=1.0
    )
...
def test_format_as_markdown_empty_day():
    ...
    daily_report = DailyReport(date=start_date, tag_summaries={}, total_hours=0.0)
