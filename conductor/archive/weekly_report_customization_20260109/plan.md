...
## Phase 2: Add Markdown and CSV Output Formats [checkpoint: 274f96f]
...
- [x] Task: Update `time_helper/cli/report_commands.py` to add `--format` option. [commit: a6888b0]
    - [x] Write Tests: Add unit tests for `report_commands.py` to verify parsing of the new `--format` argument.
    - [x] Implement Feature: Modify the `report` command function to accept the `--format` argument.
...
- [x] Task: Conductor - User Manual Verification 'Phase 2: Add Markdown and CSV Output Formats' (Protocol in workflow.md)
