...
- [ ] Task: Modify `time_helper/database.py` to allow filtering of time entries by date range and tags.
    - [ ] Write Tests: Add unit tests for `database.py` to verify correct filtering logic for date ranges and tags when querying time entries.
    - [ ] Implement Feature: Update database query methods to incorporate `start_date`, `end_date`, and `tags` for data retrieval.
