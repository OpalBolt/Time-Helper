...
def _parse_entries_by_date(data: List[dict]) -> tuple[Dict[str, List[TimeEntry]], int, str, str]:
...
def _display_dry_run_summary(entries_by_date: Dict[str, List[TimeEntry]], total_entries: int, 
                           earliest_date: str, latest_date: str) -> None:
...
def import_all_data(dry_run: bool = False, force: bool = False) -> None:
    ...
    if not force and not dry_run:
        with sqlite3.connect(db.db_path) as conn:
            ...
            if existing_count > 0:
                rprint(f"[yellow]Database already contains {existing_count:,} entries.[/yellow]")
                rprint("[yellow]Use --force to import anyway, or --dry-run to see what would be imported.[/yellow]")
                return
    ...
    entries_by_date, total_entries, earliest_date, latest_date = _parse_entries_by_date(data)
    
    rprint(f"[green]Processing {len(data)} entries...[/green]")
    
    if dry_run:
        _display_dry_run_summary(entries_by_date, total_entries, earliest_date, latest_date)
        return
...
def database_status() -> None:
    ...
    try:
        ...
        logger.info(f"Database status check: {total_entries} entries, {total_hours:.2f} hours")
