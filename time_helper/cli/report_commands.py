...
def _determine_target_week(date_str: Optional[str], week_offset: int, year: Optional[int]) -> date:
    ...
    """
    logger.debug(f"Determining target week: date_str={date_str}, offset={week_offset}, year={year}")
...
@handle_timew_errors
def export_week(week_offset: int = 0, year: Optional[int] = None, date_str: Optional[str] = None) -> None:
    ...
    """
    logger.info(f"Exporting week data: offset={week_offset}, year={year}, date={date_str}")
    ...
    rprint(f"[blue]📤 Exporting data for week of {week_start.strftime('%B %d, %Y')}...[/blue]")
    ...
    if not all_entries:
        rprint(f"[yellow]No time entries found for week of {week_start.strftime('%B %d, %Y')}[/yellow]")
        return
...
@handle_timew_errors
def generate_report(week_offset: int = 0, year: Optional[int] = None, 
                   date_str: Optional[str] = None, use_cache: bool = True) -> None:
    ...
    """
    logger.info(f"Generating report: offset={week_offset}, year={year}, date={date_str}, cache={use_cache}")
